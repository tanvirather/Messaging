...
public class OrderConsumerTests
{
    ...
    public OrderConsumerTests()
    {
        ...
        // OrderRepository needs MessagingContext. We can use Moq for the repository if its methods are virtual.
        // Let's check OrderRepository structure.

        _mockEmailService = new Mock<EmailService>(null!);
    ...
    public async Task ConsumeAsync_WhenValid_SendsEmail()
    {
        ...
        // Assert
        _mockEmailService.Verify(e => e.SendEmailAsync("test@example.com", "Subject", "Body"), Times.Once);
    }
