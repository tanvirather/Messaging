...
Create a class in `Messaging/Composers/`. Inherit from `BaseEmailComposer` to use helper methods like `ReadTemplate` and `CreateHtmlAsync`.
...
## 2. Create the Model
Create a model in `Messaging/Models/` to hold the data used by the composer and fetched by the repository.
...
Create a repository class in `Messaging/Repositories/` to fetch and project data from the database into models. The repository should accept `MessagingContext` in its constructor.

**Example: `NewEntityRepository.cs`**
```csharp
public class NewEntityRepository(MessagingContext context)
{
...
Create a message class in `Messaging/Messages/`. This class should implement `IMessage` (if applicable) and contain the data needed to identify the entity (e.g., `OrderId`).
...
To add a new email type to the `EmailController` in `Messaging.Api/Controllers/`, add a new `HttpPost` method that accepts the message and calls `QueueMessage`.
...
Create a validator in `Messaging/Validators/` to ensure the data is valid before processing.
...
Add a new `.html` template in `Messaging/Templates/`. Use placeholders like `{variableName}` for dynamic content.

## 8. Create the Consumer
Create a consumer in `Messaging/Consumers/` that implements `IConsumer<NewEmailMessage>`. This class orchestrates the validation, mapping, and sending of the email.
